...
#elif not re.match("^[A-Za-z ]+$", ride_name):
#           messagebox.showerror("Error", "Ride name can only contain alphabets.")
...
def signup():
    ...
    button_signup = tk.Button(signup_window, text="Sign Up", command=handle_signup)
...
button_signup = tk.Button(login_window, text="Sign Up", command=signup)
button_signup.pack()


label_font = font.Font(size=50, weight="bold")
label_username = tk.Label(login_window, text="Username:", font=label_font)
label_password = tk.Label(login_window, text="Password:", font=label_font)
